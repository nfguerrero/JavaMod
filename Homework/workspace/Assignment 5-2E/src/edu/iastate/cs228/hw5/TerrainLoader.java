...
public class TerrainLoader {
    ...
    //TODO: define meaningful messages for each of these. 
    private static final String kWaterError = "Incorrect coordinate fomat";
    private static final String kFlagError = "Incorrect number of flags, or illegal coordinate format";
    private static final String kBrushError = "Incorrect coordinate fomat";
    private static final String kForestError = "Incorrect coordinate fomat";
    private static final String kDirtError = "Incorrect number of coordinates, or coordinates not adjacent, or illegal coordinate format";
    private static final String kHwy2Error = "Incorrect number of coordinates, or coordinates not adjacent, or illegal coordinate format";
    private static final String kHwy4Error = "Incorrect number of coordinates, or coordinates not adjacent, or illegal coordinate format";
    private static final String kRiverError = "Incorrect number of coordinates, or coordinates not adjacent, or illegal coordinate format";
    private static final String kWallError = "Illegal coordinate format, or illegal hex dir format";
    private static final String kDefaultParseError = "Incorrect format";
    private static final String kPathError = "Incorrect number of coordinates, or coordinates not adjacent, or illegal coordinate format";
...
//        System.out.println("Processing "+keyword);
    ...
    private void readBorderPath(  ConnType type, String errMsg) throws ParseErrorException  {
       //TODO:
    	while (this.scan.hasNextCoord()) {
    		Coordinate cFrom = this.scan.nextCoord();
    		int x = cFrom.getHexX();
    		int y = cFrom.getHexY();
    		while (this.scan.hasNextDir()) {
    			HexDir dir = this.scan.nextDir();
    			int dX = dir.getDelta().x;
    			int dY = dir.getDelta().y;
    			try {
    				this.tGraph.setEdge(x, y, x+dX, y+dY, type);
    			} 
    			catch (Exception e) {
    ...
    private void readPath( ConnType type, String errMsg) throws ParseErrorException {
        //TODO: capture any exceptions and add errMsg to them before rethrowing
        // forward any valid paths (along with connector type information to 
        // tGraph.setConnectedPath().
    	Path path = scanPath(type, errMsg);
    	if (this.scan.hasNextInt()) { throw new ParseErrorException(errMsg);}
    	try {
    		this.tGraph.setConnectedPath(path, type);
    	}
    	catch (Exception e) {
    ...
    private void readCells( TerrainType terrain, String errMessage) throws ParseErrorException {
        //TODO: pass the terrain description to tGraph.
    	while (this.scan.hasNextCoord()) {
    		Coordinate coord = this.scan.nextCoord();
    		this.tGraph.setTerrain(coord.getHexX(), coord.getHexY(), terrain);
    	}
    	if (this.scan.hasNextInt()) { throw new ParseErrorException(errMessage); }
    }
    ...
    private List<Coordinate> scanCoordList(){
    	...
    	while (this.scan.hasNext()) {
    		if (this.scan.hasNextCoord()) {
    			coords.add(this.scan.nextCoord());
    		}
    		else {Object trash = this.scan.next();}
    	}
        return coords;
    }
    /**
    ...
    private Coordinate[] readTwo(String errMessage) throws ParseErrorException {
    	...
    	if (this.scan.hasNextCoord()) {
    		coords[0] = this.scan.nextCoord();
    	} else { throw new ParseErrorException(errMessage);}
    	if (this.scan.hasNextCoord()) {
    		coords[0] = this.scan.nextCoord();
    	} else { throw new ParseErrorException(errMessage);}
    	if (this.scan.hasNextCoord() || this.scan.hasNextInt()) { throw new ParseErrorException(errMessage);}
        return coords;
    }
