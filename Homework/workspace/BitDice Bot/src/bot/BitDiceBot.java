...
public class BitDiceBot 
{
	...
	{		
		...
		//String lowFor = betNorm("Low Fourth", 100, 752499, 4, .4);
		//System.out.println(lowFor);
		
		//String lowFive = betNorm("Low Fifth", 100, 801999, 5, .275);
		//System.out.println(lowFive);
		
		//String lowFive2 = betNorm("Low 5-2", 100, 801999, 5, .3);
		//System.out.println(lowFive2);
		
		//String lowFive3 = betNorm("Low 5-3", 100, 801999, 5, .325);
		//System.out.println(lowFive3);
		
		//String lowSix = betNorm("Low 6-1", 100, 834999, 6, .25);
		//System.out.println(lowSix);
		
		//String lowSix2 = betNorm("Low 6-2", 100, 834999, 6, .2);
		//System.out.println(lowSix2);
		
		//String lowSix3 = betNorm("Low 6-3", 100, 834999, 6, .15);
		//System.out.println(lowSix3);
		...
		*/
		...
		String vid1 = betNorm2("Vid1", 40, 752499, 4, .4);
		System.out.println(vid1);
		
		String vid1n = betNorm("Vid1N", 40, 752499, 4, .4);
		System.out.println(vid1n);
		
		String vid1l = betLoop("Vid1L", 40, 752499, 4, .4);
		System.out.println(vid1l);
	}
	
	public static String betLoop(String name, int base, int chance, double mult, double dLoss){
		int win = 0; long moves = 0; int count = 0;
		boolean lost = false; double lostx2 = 0;
		...
		for (int i = 0; i < 10000; i++){
			...
			{
				int[] out = betHelp(gen, bal, base, chance, mult, dLoss);
				bal = out[0];
			}	
			if (bal >= 3000000){win++;}
		}
	...
	public static int[] betHelp(Random gen, int cur, int base, int chance, double mult, double dLoss){
		int bets = 0;
		int delta = base; int bal = cur;
		
		while (bal > 0 && bal < cur+100000){
	...
	public static String betNorm2(String name, int base, int chance, double mult, double dLoss){
		int win = 0;
		long moves = 0;
		boolean lost = false;
		double lostx2 = 0;
		...
		for (int i = 0; i < 10000; i++){
			...
			if (bal>=1600000){moves+=bets;win++;lost=false;}
			else {if(lost){lostx2++;} lost = true;}
		}
		System.out.println(" ");
		double total = Math.round(10000*(win/10000.0))/100.0;
